...
router.get("/friends/pending/:userId", async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from("friends")
      .select("friendship_id, requester_id, status, users:requester_id(username,email)")
      .eq("recipient_id", userId)   // only requests sent to current user
      .eq("status", "pending");
...
router.post("/friends/respond", async (req, res) => {
  const { friendship_id, status } = req.body; // status = "accepted" or "rejected"
...
router.get("/friends/:userId", async (req, res) => {
  ...
  try {
    ...
    // Map to get only the other user
